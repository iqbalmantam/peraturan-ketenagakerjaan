...
with tab1:
    ...
    # Form input chat berada di atas (tidak menempel di bawah)
    ...
    if submit_btn and user_query:
        ...
        with st.spinner("Menganalisis seluruh isi dokumen..."):
            try:
                # Menggunakan model gemini-3.6-flash sesuai standar terbaru
    ...
    # Tampilkan Riwayat Percakapan di bawah kolom input
    st.markdown("---")
...
with tab2:
    st.subheader("📖 Teks Lengkap Peraturan Perusahaan")
    st.markdown("Anda dapat membaca langsung atau meninjau isi teks dokumen PDF yang diunggah di sini.")
    if pdf_text:
        st.text_area("Isi Dokumen PDF:", value=pdf_text, height=600)
    else:
        st.error(f"File PDF `{TARGET_PDF}` tidak ditemukan di repositori.")
